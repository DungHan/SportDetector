...
### Sport classification: fine-tuned lightweight classifier, not zero-shot ImageNet, cached per source (superseded)
**Superseded**: after further discussion with the user, this decision was replaced by "Sport classification: CLIP/SigLIP zero-shot" below — no fine-tuned classifier or labeled training set is being built for v1. Kept here for history; the training-data and caveat notes in this section still apply as general context (e.g., the caveat about not reusing broadcast/game reasoning for court-keypoint detection).
...
### Sport classification: CLIP/SigLIP zero-shot, no fine-tuning, cached per source
`NBA.Vision` classifies a source's sport by running a CLIP-family (or SigLIP) vision encoder on the frame and comparing it against a fixed set of **precomputed text-prompt embeddings**, one per registered sport (e.g., `"a basketball game"`, `"a soccer match"`, `"a tennis match"`) — the top-scoring prompt (above threshold) becomes the reported sport. This replaces the previously planned fine-tuned classifier (see the superseded decision above) — no labeled training set or fine-tuning step is required for v1.

**Why**: this is a genuinely different technique from the ImageNet-1k zero-shot approach already rejected above — ImageNet's zero-shot classes are single-word object labels learned from object-centric close-ups (poor fit for wide broadcast/game scenes), whereas CLIP/SigLIP are contrastively trained on billions of full-scene image/caption pairs and are used here with whole-scene sentence prompts, which is the regime they were built for. Their pretraining diversity (real photos, broadcast footage, game screenshots, illustrations, etc.) also makes them inherently more robust to the broadcast-vs-game-rendering style gap than a small classifier fine-tuned on a few hundred of the project's own screenshots would be — removing the need to source and label that dataset at all.

**How it fits the existing architecture**: implemented as a new `ISportClassifier` (e.g., `ClipZeroShotSportClassifier`) — no interface change. Only the vision encoder is exported/shipped as `.onnx` and run at inference time (one pipeline instance, per the `inference/onnx-runtime` pattern); the text encoder runs **once, offline**, over the fixed prompt list, and its output embeddings are baked into the app as a constant matrix, so no text-encoder model or tokenizer ships at runtime. Classification still runs once per selected capture source (not per frame) and is cached in `SourceProfile`, exactly per the existing "detect once per source, reuse" rule — this change only replaces *how* the per-source classification is computed, not when.

**Must recalibrate the confidence threshold**: the acceptance threshold (used to gate auto-acceptance vs. reporting "unknown") was chosen for a fine-tuned softmax classifier's output distribution and does **not** carry over — CLIP/SigLIP similarity scores have different distribution characteristics (SigLIP's sigmoid-based scores in particular behave differently from a softmax-over-classes). The threshold must be re-derived empirically against real captured frames (a mix of broadcast and game-source screenshots) before this ships, not assumed from the old value.

**Alternative considered**: the fine-tuned lightweight classifier (previous decision, now superseded). Rejected in favor of this approach specifically because it removes the need to collect and label a training set at all, and is expected to generalize better across the broadcast/game rendering gap — the user confirmed this trade-off directly.

**Non-Goal carried over**: this decision is scoped to the *classification* step only. It does not extend to court-keypoint detection (`vision/court-calibration`) — see the "Caveat" note in the superseded decision above, which still holds: keypoint localization needs its own per-sport trained model regardless of how sport classification is done.
...
- ~~Exact composite key strategy for `SourceProfile` identity (window title vs. process name vs. user-assigned nickname)~~ **Resolved during live testing**: the initial implementation (`SourceIdentity.DeriveKey`, `src/NBA.App/Services/SourceIdentity.cs`) keyed on kind+process-name only, which collapsed every window of the same app (e.g. every Chrome window/tab) into one cache entry - confirmed live when a Chrome window showing a real NBA broadcast silently reused a stale "unknown" sport classification cached from an unrelated Chrome window selected earlier. Fixed by adding display name (window title) to the key. Accepts the already-known trade-off from the Risks section below (a YouTube tab's title changes with the video, so its cached classification/calibration won't be reused across videos) as strictly preferable to two unrelated windows silently sharing one cached result. A user-assigned nickname is still not implemented.
