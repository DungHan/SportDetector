...
- Input: `images`, shape `[1, 3, 1280, 1280]` (NCHW, RGB, values scaled to `[0, 1]`) — note the larger `1280` export size, unlike the generic `yolov8n.onnx` verification below (`640`). `OnnxPlayerDetector`'s `inputSize` must be passed as `1280` at the `App.axaml.cs` call site for this file; the library's own constructor default (`640`) is left unchanged since it's also relied on by unit tests using a small synthetic fixture.
- Output: `output0`, shape `[1, 13, N]` — 4 box channels (`cx, cy, w, h`, in `[0, 1280]` input-pixel space) followed by 9 per-class confidence scores, no NMS baked in (same raw Ultralytics export shape `OnnxPlayerDetector` already expects).
- Embedded class-name metadata (`names` key, verified by inspection): `{0: 'Ball', 1: 'Hoop', 2: 'Period', 3: 'Player', 4: 'Ref', 5: 'Shot Clock', 6: 'Team Name', 7: 'Team Points', 8: 'Time Remaining'}`. **`Player` is class index `3`** — this is the index `App.axaml.cs` passes as `OnnxPlayerDetector`'s player-class-channel constructor parameter, so only actual players (not referees, the ball, or scoreboard regions) are detected/tracked. `Ref` (index `4`) is deliberately never read, so officials no longer produce detections at all (see `add-team-color-track-gating`'s design.md).
